...
import { 
  MoreHorizontal, MapPin, Calendar, ExternalLink, Heart, 
  Video, Wand2, Image as ImageIcon, CheckCircle2, 
  Search, Clock, Zap, Briefcase, Camera, Music, AlertCircle, Plus, Globe
} from 'lucide-react';
...
const Weddings: React.FC<Props> = ({ branch }) => {
  ...
  const getStatusColor = (status: TaskStatus) => {
    switch (status) {
      case 'Completed': return 'text-emerald-500 bg-emerald-500/10 dark:bg-emerald-500/20';
      case 'In Progress': return 'text-blue-500 bg-blue-500/10 dark:bg-blue-500/20';
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar border-b border-slate-200 dark:border-white/5">
      ...
      {loading ? (
        <div className="grid grid-cols-1 gap-6 opacity-50">
           {[1,2].map(i => <div key={i} className="h-64 glass animate-pulse rounded-[2.5rem]"></div>)}
        </div>
      ) : (
